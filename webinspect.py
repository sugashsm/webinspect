...
def banner():
 ...
 __          __  _    _____                           _   
 \ \        / / | |  |_   _|                         | |  
  \ \  /\  / /__| |__  | |  _ __  ___ _ __   ___  ___| |_ 
   \ \/  \/ / _ \ '_ \ | | | '_ \/ __| '_ \ / _ \/ __| __|
    \  /\  /  __/ |_) || |_| | | \__ \ |_) |  __/ (__| |_ 
     \/  \/ \___|_.__/_____|_| |_|___/ .__/ \___|\___|\__|
                                     | |                  
                                     |_|                  
...
def export_profile(profile_id):
    ...
    print('\t[+] Creating webinspect.json : ', end='')
    try:
        metadata = {
        ...
        with open(os.path.join(final_path, 'webinspect.json'), 'w') as outfile:  
...
def arg_history(args):
    ...
    if args.export != None:
        ...
        if export_type == 'html':
            ...
            try:
                if args.profile != None:
                    output_filename = 'webinspect_profile_{}_{}.csv'.format(str(args.profile[0]),current_time)
                else:
                    output_filename = 'webinspect_profiles_{}.csv'.format(current_time)
...
def arg_profiles(args):
    ...
    print('\nUsage : webinspect.py history --profile {ProfileID}')
...
parser.add_argument('-v', '--version', action='version', version='[+] webinspect current version is {}.'.format(str(__version__)))
